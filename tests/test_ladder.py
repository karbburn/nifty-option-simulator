...
def test_target_15_after_run_up():
    path = [100, 101, 105, 110, 115.0000001, 116]
    result = simulate(path, 100)
    assert result.exit_reason == "target_15"
    assert result.exit_index == 4
    assert result.exit_premium == pytest.approx(115)
    assert result.floor_level == pytest.approx(0.10)
    at_floor = simulate(path, 100, fill_mode="at_floor")
    assert at_floor.exit_reason == "target_15"
    assert at_floor.exit_index == 4
    assert at_floor.exit_premium == pytest.approx(115)


def test_floor_3_banked_then_dip():
    result = simulate([100, 103, 102.9], 100)
    assert result.exit_reason == "floor_3"
    assert result.exit_index == 2
    assert result.exit_premium == pytest.approx(102.9)
    at_floor = simulate([100, 103, 102.9], 100, fill_mode="at_floor")
    assert at_floor.exit_premium == pytest.approx(103)
...
def test_sl_wins_over_banked_floor():
    result = simulate([100, 105, 92.9], 100)
    assert result.exit_reason == "SL"
    assert result.exit_index == 2
    assert result.exit_premium == pytest.approx(92.9)
    assert result.floor_level == pytest.approx(0.05)
    at_floor = simulate([100, 105, 92.9], 100, fill_mode="at_floor")
    assert at_floor.exit_premium == pytest.approx(93.0)
    floor_3_banked = simulate([100, 103, 92], 100)
    assert floor_3_banked.exit_reason == "SL"
    assert floor_3_banked.exit_premium == pytest.approx(92)
...
def test_dip_above_banked_floor_then_target():
...
def test_inexact_floats_decisive():
    result = simulate([100, 103.0000001, 102.995], 100)
    assert result.exit_reason == "floor_3"
    assert result.exit_index == 2
    assert result.exit_premium == pytest.approx(102.995)
